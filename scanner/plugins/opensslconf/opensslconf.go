...
func (p *Plugin) UpdateBOM(fs filesystem.Filesystem, bom *cdx.BOM) error {
	...
	// lazy-load ciphersuite DB once
	if err := loadCipherSuitesDB(); err != nil {
		log.WithError(err).Warn("Failed to load ciphersuites.json; TLS mapping will be skipped")
	}
	...
	for _, f := range found {
		...
		if len(versions) == 0 || len(suites) == 0 || cipherSuiteDB == nil {
			continue
		}
		for _, v := range versions {
			algoComps, protoComp, depMap := buildProtocolComponents(v, suites, f.path)
...
func loadCipherSuitesDB() error {
	cipherSuiteOnce.Do(func() {
		...
		// Parse the array of single-key objects
...
func buildProtocolComponents(version string, suiteNames []string, srcPath string) ([]cdx.Component, *cdx.Component, map[cdx.BOMReference][]string) {
	...
	// protocol component
	protoRef := fmt.Sprintf("crypto/protocol/tls@%s", version)
	protoName := fmt.Sprintf("TLSv%s", version)
	proto := cdx.Component{
		...
		CryptoProperties: &cdx.CryptoProperties{
			...
			ProtocolProperties: &cdx.CryptoProtocolProperties{
				...
				CryptoRefArray: func() *[]cdx.BOMReference { c := make([]cdx.BOMReference, len(refOrder)); copy(c, refOrder); return &c }(),
			},
		},
	...
	deps := make([]cdx.Dependency, 0)
	protoDepends := make([]string, 0, len(refOrder))
	for _, r := range refOrder {
		protoDepends = append(protoDepends, string(r))
	}
	deps = append(deps, cdx.Dependency{Ref: protoRef, Dependencies: &protoDepends})
	...
	for _, comp := range algoComps {
		if comp.CryptoProperties != nil && comp.CryptoProperties.AlgorithmProperties != nil {
			if strings.Contains(strings.ToUpper(comp.Name), "WITH") {
				// find hash by parsing name prefix before 'with'
				parts := strings.Split(strings.ToUpper(comp.Name), "WITH")
				if len(parts) >= 1 {
					h := strings.TrimSpace(parts[0])
					href := algorithmRefForName(h)
					if href != "" {
						deps = append(deps, cdx.Dependency{Ref: comp.BOMRef, Dependencies: &[]string{href}})
					}
				}
			}
		}
	...
	// convert deps slice to map for AddDependencies
	depMap := make(map[cdx.BOMReference][]string)
	for _, d := range deps {
		if d.Dependencies != nil && len(*d.Dependencies) > 0 {
			depMap[cdx.BOMReference(d.Ref)] = append(depMap[cdx.BOMReference(d.Ref)], (*d.Dependencies)...)
		}
	}
...
func extractAlgorithmsFromSuite(cs *cipherSuite) []string {
	...
	// Auth algorithm (if different from kex)
